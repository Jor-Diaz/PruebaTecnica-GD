...
export default function PaginationGD({
  post_estaciones_list_page,
  estacionesArray,
}) {
  const [active, setActive] = useState(1);
  const [listingsPerPage, setListingsPerPage] = useState(50);
  const [currentPage, setCurrentPage] = useState(1);

  const visitPage = (page) => {
    setCurrentPage(page - 1);
    setActive(page - 1);
    post_estaciones_list_page(page - 1);
  };
  ...
  const previous_number = () => {
    if (currentPage !== 1) {
      ...
      post_estaciones_list_page(currentPage - 1);
    }
  };

  const next_number = () => {
    window.scrollTo(0, 0);
    if (currentPage !== Math.ceil(estacionesArray.length / 3)) {
      setCurrentPage(currentPage + 1);
      setActive(currentPage + 1);
      post_estaciones_list_page(currentPage + 1);
    }
